...
{
	char * anchor_id = (char *) malloc(256);

	strcpy(anchor_id, anchorstring);

	int i, j;
	for (i = 0, j = 0; anchor_id[i] != 0; i++) {
		anchor_id[i] = tolower(anchor_id[i]);

		// Check space
		if (anchor_id[i] == ' ') {
			anchor_id[i] = '-'; // Change space to hyphen
		}

		// Check allow character. number, letter, underscore or hyphen(space)
		// All special character will be remove
		if ((anchor_id[i] >= 'a' && anchor_id[i] <= 'z') || (anchor_id[i] >= '0' && anchor_id[i] <= '9') || anchor_id[i] == '-' || anchor_id[i] == '_') {
			anchor_id[j] = anchor_id[i];
			j++;
		}
	}
	anchor_id[j] = '\0';
